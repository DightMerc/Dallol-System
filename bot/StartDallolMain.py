...
@dp.message_handler(commands=['setnumber'], state="*")
async def process_start_command(message: types.Message, state: FSMContext):
    user = message.from_user.id

    text = "Для получения доступа к статистике отправьте свой номер телефона"
    await bot.send_message(user, text, reply_markup=keyboards.ContactKeyboard(user))

    await User.setNumber.set()


@dp.message_handler(state=User.setNumber, content_types=types.ContentType.CONTACT)
async def user_contact_handler(message: types.Message, state: FSMContext):
    ...
    if user!=message.contact.user_id:
        text = "Нужен именно ваш номер телефона"
        await bot.send_message(user, text, reply_markup=keyboards.ContactKeyboard(user))
    else:
        if await client.SetPhone(user, phone):
            text = "Спасибо! Теперь Вы можете просмотреть статистику на lumencorp.net:8002/. Узнайте Ваш пароль у администратора"
            await bot.send_message(user, text, reply_markup=keyboards.ContactKeyboard(user))
        else:
            text = "К сожалению, что-то пошло не так. Обратитесь к администратору"
            await bot.send_message(user, text, reply_markup=keyboards.ContactKeyboard(user))
...
async def process_start_command(message: types.Message, state: FSMContext):
    ...
    await bot.send_video(user, InputFile(os.path.join(os.getcwd(), "Instructions", "after_start.mp4")))
...
async def back_handler(message: types.Message, state: FSMContext):
    ...
    if current_state in ["Sale:started", "Rent:started", "OnlineSearch:type_choosen", "OnlineSearch:title_added", "OnlineSearch:region_added", "OnlineSearch:region_added"]:
        ...
        await bot.send_video(user, InputFile(os.path.join(os.getcwd(), "Instructions", "after_sale.mp4")))
        ...
        text = Messages(user)['choose_action_sale']
        ...
        markup = keyboards.SaleAndRentKeyboard(user)
        await bot.send_message(user, text, reply_markup=markup)
    elif current_state in "OnlineSearch:announcement":
        ...
        text = Messages(user)['choose_action_rent']
        markup = keyboards.SaleSearchAndannouncementKeyboard(user)
        await bot.send_message(user, text, reply_markup=markup)

    elif current_state in ["Rent:main_region_added","Sale:main_region_added","Rent:title_added", "Rent:region_added", "Rent:reference", "Rent:OnlineSearch.","Sale:title_added", "Sale:region_added", "Sale:reference", "Sale:OnlineSearch"]:
        ...
        async with state.proxy() as data:
            ...
            try:
                ...
                if _type!="search":
                    ...
                    markup = keyboards.EditOnlineSearchMarkup(user_data, user)
...
async def user_contact_handler(message: types.Message, state: FSMContext):
    ...
    phone = message.contact.phone_number
    # await client.SetPhone(user, phone)
    ...
    photoes = os.listdir(os.getcwd()+"/Users/" + str(user)+"/")
    await bot.send_video(user, InputFile(os.path.join(os.getcwd(), "Instructions", "how_to_edit.mp4")))
...
async def callback_search_handler(callback_query: types.CallbackQuery, state: FSMContext): 
    ...
    if orders.count!=0:
        ...
        for order in orders.get_page(num):
            await client.TickAdd(order.id)
            _ann_number = order.id
            _type = order.type
            _property = order.property
            _title = order.title
...
async def callback_edit_handler(callback_query: types.CallbackQuery, state: FSMContext):   
    ...
    if not content=="cancel":
        ...
        if content == "property":
            ...
            markup = keyboards.MainRegionKeyboard(user)
...
async def next_button_handler(message: types.Message, state: FSMContext):
    ...
    if my_state in "Rent:reference":
        ...
        if _type == "Участок земли":
            ...
            text = Messages(user)["land_started"]
            markup = keyboards.BackKeyboard(user)
...
async def language_handler(message: types.Message, state: FSMContext):
    ...
    if recieved_text == "Русский язык" or recieved_text == "Ўзбек тили":
        ...
        await bot.send_video(user, InputFile(os.path.join(os.getcwd(), "Instructions", "after_lan.mp4")))
...
async def menu_handler(message: types.Message, state: FSMContext):
    ...
    elif recieved_text in ["Помощь", "Ёрдам"]:
...
async def online_order_handler(message: types.Message, state: FSMContext):
    ...
    if recieved_text in ["Сделать заказ", "Буюртма бериш"]:
        ...
        if sub_type == "rent":
            ...
            text = Messages(user)['choose_action_search']
...
async def Search_type_choosen_handler(message: types.Message, state: FSMContext):
    ...
    if recieved_text in keyboards.SaleSearchAndannouncementKeyboardList:
        ...
        MessageDict = {'🏠 Участок': Messages(user)['area_search'], '🏬 Квартира': Messages(user)['flat_search'], '🏡 Участок земли': Messages(user)['land_search'], '🏗 Коммерческая недвижимость': Messages(user)['free_area_search'],
                        '🏠 Ховли': Messages(user)['area_search'], '🏡 Ер': Messages(user)['land_search'], '🏗 Тижорат кўчмас мулки': Messages(user)['free_area_search'] }
...
async def Search_region_handler(message: types.Message, state: FSMContext):
    ...
    await OnlineSearch.main_region_added.set()

    async with state.proxy() as data:
        data["search main_region"] = recieved_text

    text = Messages(user)["title_added"]
    markup = keyboards.RegionKeyboard(user, recieved_text)
    await bot.send_message(user, text, reply_markup=markup)

@dp.message_handler(state=OnlineSearch.main_region_added)
...
async def sale_region_handler(message: types.Message, state: FSMContext):
    ...
    await Rent.main_region_added.set()
...
@dp.message_handler(state=Rent.main_region_added)
async def sale_region_handler(message: types.Message, state: FSMContext):
...
async def data_search_handler(message: types.Message, state: FSMContext):
    ...
    if recieved_text in ["Цена", "Нарх"]:
        ...
        if _property == "Квартира":
            ...
            if _type == "sale":
                text = Messages(user)["price_list_common"]
                numbers = []
                for a in range(1, 21):
                    numbers.append(a)
                markup = keyboards.PriceSetKeyboard(user, numbers, _property)
                ...
                text = Messages(user)["price_list2_common"]
                numbers = []
                for a in range(1, 21):
                    numbers.append(a)
                markup = keyboards.PriceSetKeyboard(user, numbers, _property)
...
@dp.message_handler(state=Search.main_region)
async def sale_region2_handler(message: types.Message, state: FSMContext):
...
async def sale_region_handler(message: types.Message, state: FSMContext):
    ...
    await Sale.main_region_added.set()
...
@dp.message_handler(state=Sale.main_region_added)
async def sale_region_handler(message: types.Message, state: FSMContext):
...
async def text_phone_handler(message: types.Message, state: FSMContext):
    ...
    if data.isdigit():
        ...
        # await client.SetPhone(user, int(phone))
